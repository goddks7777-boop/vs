(() => {
  ...
  async function load() {
    ...
    const posRows = positions.map(([symbol, p]) => {
      const x = byCode[symbol] || {}, entry = Number(p.entry), target = entry * 1.06, current = Number(x.price || entry), gain = (current / entry - 1) * 100;
      return `<tr><td>${esc(p.market)}</td><td><b>${esc(symbol)}</b> ${esc(p.name)}</td><td>${won(entry)}</td><td><b>${won(target)}</b> <span class="target-rate">(+6%)</span></td><td>${won(current)}</td><td class="${cls(gain)}">${gain >= 0 ? '+' : ''}${gain.toFixed(2)}%</td><td>${esc(x.score ?? p.score ?? '—')}/6</td></tr>`;
    }).join('') || '<tr><td colspan="7">현재 보유 종목이 없어 현금 대기 중입니다.</td></tr>';
    ...
    const tradeRows = [...normalizedTrades].reverse().map(t => {const sold = t.side === 'SELL', rate = sold && t.returnPct != null ? Number(t.returnPct) : null; return `<tr><td>${stamp(t.time)}</td><td class="${t.side === 'BUY' ? 'up' : 'down'}">${esc(t.side)}</td><td>${esc(t.market)}</td><td><b>${esc(t.symbol)}</b> ${esc(t.name)}</td><td>${won(t.entryPrice ?? t.price)}</td><td>${sold ? won(t.price) : '—'}</td><td class="${rate == null ? '' : cls(rate)}">${rate == null ? '보유 중' : `${rate >= 0 ? '+' : ''}${rate.toFixed(2)}%`}</td><td class="${t.pnl == null ? '' : cls(t.pnl)}">${t.pnl == null ? '—' : `${Number(t.pnl) >= 0 ? '+' : ''}${won(t.pnl)}`}</td><td>${esc(t.reason)}</td></tr>`;}).join('') || '<tr><td colspan="9">아직 체결이 없습니다.</td></tr>';
    ...
    const heldHeader = main.querySelector('.panel:first-of-type thead tr');
    if (heldHeader) {const targetHead = document.createElement('th'); targetHead.textContent = '목표가'; heldHeader.insertBefore(targetHead, heldHeader.children[3]);}
    const tradePanel = [...main.querySelectorAll('.panel')].find(panel => panel.querySelector('h2')?.textContent.includes('가상 체결 기록'));
    if (tradePanel) tradePanel.querySelector('thead tr').innerHTML = '<th>시각</th><th>구분</th><th>시장</th><th>종목</th><th>매수가</th><th>매도가</th><th>수익률</th><th>실현손익</th><th>이유</th>';
    const rules = main.querySelector('.panel:last-child');
    if (rules) rules.querySelector('h2').insertAdjacentHTML('afterend', '<p><b>목표가:</b> 매수가 대비 +6%입니다. 보유 표에서 종목별 계산 가격을 바로 확인할 수 있습니다.</p>');
  }
  const style = document.createElement('style');
  style.textContent = '.live-status{border-radius:12px;padding:12px 14px}.live-status.ok{background:#29312f;border:1px solid #61706a;color:#e3ece8}.live-status.warn{background:#3a2b0b;border:1px solid #b17b16;color:#ffe4a3}.journal{padding:12px 0;border-bottom:1px solid #444}.journal-time{display:flex;gap:12px;margin-bottom:6px}.journal-time span{color:#aaa}.target-rate{color:#a9c9bc;font-weight:800;white-space:nowrap}';
