...
CFG={
 "scalp":{"label":"단타","setup":"추세 돌파형","budget":1_500_000,"max":3,"maxHours":24,"stopAtr":1.2,"targetAtr":2.0},
 "swing":{"label":"스윙","setup":"MA120 반등형","budget":2_000_000,"max":3,"maxHours":240,"stopAtr":1.6,"targetAtr":3.0},
 "long":{"label":"장타","setup":"매집 전환형","budget":2_000_000,"max":3,"maxHours":2160,"stopAtr":2.2,"targetAtr":5.0},
}
...
for x in liquid:
 try:
  ...
  for kind in ("추세 돌파형","MA120 반등형","매집 전환형"):
...
for key,cfg in CFG.items():
 ...
 ready=[x for x in candidates if x["strategy"]==cfg["setup"] and x["state"]=="확인" and x["symbol"] not in p["positions"]]
 ...
 value=p["cash"]+sum(z["qty"]*prices.get(s,z["entry"])*(1-FEE-SLIP) for s,z in p["positions"].items());p["curve"].append({"time":NOW.isoformat(timespec="seconds"),"value":value});p["curve"]=p["curve"][-2000:];p["journal"].append({"time":NOW.isoformat(timespec="seconds"),"actions":actions or ["새 가상체결 없음"]});p["journal"]=p["journal"][-500:];p["stats"]=stats(p,value)
