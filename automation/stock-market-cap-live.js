(() => {
  ...
  fetch('stock_data/market_caps.json?v=' + Date.now(), {cache:'no-store'}).then(r => r.json()).then(payload => {
    ...
    if (summary) summary.insertAdjacentHTML('beforeend', `<span>시총 갱신 <b>${esc(payload.updatedAt)}</b></span>`);
  }).catch(() => {});
})();
