...
latest_path=ROOT/"monitor_data"/"latest.json";market_now={"items":[],"breadth":{},"source":"Upbit KRW market 24h snapshot"}
if latest_path.exists():
    snap=json.loads(latest_path.read_text(encoding="utf-8"));items=sorted(snap.get("items",[]),key=lambda x:x.get("value24",0),reverse=True);total=sum(x.get("value24",0) for x in items);top=items[:40];btc=next((x for x in items if x.get("symbol")=="BTC"),None);alts=[x for x in items if x.get("symbol") not in ("BTC","USDT","USDC")]
    market_now={"snapshotTime":snap.get("collectedAt") or snap.get("time"),"source":"Upbit KRW market 24h snapshot","sizeMetric":"24시간 누적 거래대금","colorMetric":"24시간 등락률","items":[{"symbol":x["symbol"],"name":x["name"],"price":x["price"],"change24":x["change24"],"value24":x["value24"]} for x in top],"breadth":{"listed":len(items),"up":sum(x.get("change24",0)>0 for x in items),"down":sum(x.get("change24",0)<0 for x in items),"flat":sum(x.get("change24",0)==0 for x in items),"top10SharePct":sum(x.get("value24",0) for x in items[:10])/total*100 if total else 0,"btcChangePct":btc.get("change24",0) if btc else 0,"altMedianChangePct":statistics.median([x.get("change24",0) for x in alts]) if alts else 0}}
payload={"updatedAt":datetime.now(KST).isoformat(timespec="seconds"),"source":"Upbit KRW-BTC completed daily candles","firstDate":dates[0],"lastDate":dates[-1],"days":len(bars),"forwardDays":30,"regimeRule":"당시 종가와 200일선 위치 + 200일선 20일 기울기","current":{"date":current["date"],"price":current["price"],"regime":current["regime"],"rsi":current["rsi"],"ma200GapPct":current["ma200GapPct"],"ma200SlopePct":current["ma200SlopePct"],"drawdownPct":current["drawdownPct"],"cyclePosition":current["cyclePosition"]},"marketNow":market_now,"regimeAnalysis":analysis,"multiYearWindows":windows,"series":[{"date":r["date"],"price":r["price"],"regime":r["regime"]} for r in rows[::7]],"actualOrders":0}
